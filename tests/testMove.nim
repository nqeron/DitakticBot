...
func roundtrip[R](typ: R, val: string, expanded: bool = false): bool =
    if R is Game:
        
        var (playType, move, err) = parseMove(val, typ.board.len)
        if ?err:
             return false

        case playType
        of PlayType.undo: return "undo" == val
        of PlayType.move: return move.ptnVal(typ.board.len, expanded) == val
